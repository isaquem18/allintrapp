...
export function SideBarNavigationItemComponent({
  state,
  navigation,
  descriptors,
}: Props) {
  const selectedAnimatedOption = useSharedValue(0);
  ...
  const animateX = (index: number) => {
    ...
    selectedAnimatedOption.value = withTiming(120 * index, {
      duration: 200,
    });
  };
  ...
  const selectedAnimatedOptionStyle = useAnimatedStyle(() => ({
    transform: [{ translateY: selectedAnimatedOption.value }],
  }));
  ...
  return (
    <>
      ...
      <Styles.AnimatedView style={[selectedAnimatedOptionStyle]}>
